...
public class RMA_GenericUsages_Utility extends rmaseleniumtestscripts.RMA_TC_BaseTest{
	...
	//ModifyBy			: 0.1 - RenuVerma-10-07-2016 current format as MMddyyyy
	// ============================================================================================
	...
	//ModifyBy			: 0.1 - RenuVerma-10-13-2016 current format as hhmm                          
	//============================================================================================
	...
	{
		try {
			WebDriverWait wait = new WebDriverWait(driver, 15);		// Pls Don't comment out dynamic wait.	
	...
	public static boolean RMA_WindMsgHandlerAndTextVerify_Utility(String StrAccept, String CompareFlg, String ExpMsg, String PopUpVerifyMessage, int logval)throws Exception, Error
	{
		...
		try {
			...
			{
				System.out.println(StrAlertMessage+"and"+ExpMsg);
	...
	public static void RMA_DynamicFrameSwitching (String text) throws Exception, Error {
		try {
			...
			wait.until(ExpectedConditions.frameToBeAvailableAndSwitchToIt(RMA_FrameNavigation(text)));
	...
	// Modify By	:0.1 - RenuVerma-02-201-2017 Function Name changed as this function can also use for save file
	//============================================================================================
	...
	//Modify By			: 0.1 - RenuVerma-01-23-2017 Added Dynamic wait as per window count availability
	//============================================================================================
	...
	//Modify By		: 0.1 - RenuVerma This function is updated as per Mahendra's Input to navigate any desired frame after fail/pass
	//============================================================================================
	...
	/*
	public static void RMA_ConditionalFrameSwitching (String FramesIDs_Names) throws Exception, Error {
		try {
			String []FrameName_ID= FramesIDs_Names.split(";");
			String CurrentFrameID= RMA_GetCurrentFrameID();
			if(CurrentFrameID.equalsIgnoreCase(FrameName_ID[FrameName_ID.length-1])){// Check Last frame ID is equal to the current frame id
				System.out.println("Not Required Frame Switching as Application Is In Required Frame"+ CurrentFrameID);// Not need to switch 
			}
			else{

				while(!CurrentFrameID.equalsIgnoreCase("TopWebFrame")){// Loop continues till no frame available on the screen
					driver.switchTo().parentFrame();// switch to parent frame
					CurrentFrameID= RMA_GetCurrentFrameID();// 
				}					
				for(int i=0; i<=FrameName_ID.length-1; i++){// Continue frame switching till the given input IDs/Names Of the frame
					RMA_DynamicFrameSwitching(FrameName_ID[i]);
					CurrentFrameID= RMA_GetCurrentFrameID();
					System.out.println("Frame Switching Done As User Input::"+ CurrentFrameID);
				}						
			}
		}catch (org.openqa.selenium.TimeoutException t)
		{
			...
			System.out.println("Since It Is An Conditional Frame Switching Function No Frame Found Exception Is Catched" );
			throw(ex);
		}
		catch (Exception|Error e) {
			System.out.println("Error Occurred In Method RMA_ConditionalFrameSwitching While Switching" +" " + FramesIDs_Names+ " "+"Frame :"+  " "+e.getMessage());
			throw(e);
		}
	}*/
	//============================================================================================
	...
	public static void RMA_CloseAllWindowOtherThanRMA() throws Exception,Error
	{
		...
		{   
			...
			Iterator<String>windowiterator = strWindowHandles.iterator(); //Entire Set Of The Present Window Handles Is Iterated
			//System.out.println("Window Handle Size In RMA_CloseAllWindowOtherThanRMA Method Is : "+" " + strWindowHandles.size());
			...
			{   
				...
				while (windowiterator.hasNext())
				{
...

					{
						...
						{
							driver.switchTo().window(ChildWindow);
							//System.out.println("Window Handle In RMA_CloseAllWindowOtherThanRMA Is :"+" "+ChildWindow);
							driver.close();
	...
	//Description  		: To Retrieve The Past/Future System Date in "MMddyyyy"
	//Input Parameter 	: Past: '-' sign in front of month, Future: no sign in front of month
	//Revision			: 0.0 - RenuVerma-10-21-2015                                 
	// ============================================================================================
	public static String RMA_PastFutureDateInfo_Utility(int NumberOfMonth, int NumberOfDay, int NumberOfYear) throws Exception,Error
	{
		String PFDate = null;
		try
		{
			...
			PFDate=  new SimpleDateFormat("MMddyyyy").format(PastFutureDate);
