...
//Modify By		    : 0.0 -RenuVerma-02-03-2017 //As Suite Name given by XML Files through Parameterization
//========================================================================================================================================================================================================================================================================
{
	...
	public static String RMAApp_ExtentReports;
	public static ExtentReports RMA_ExtentReportsInstance_Utility() throws Exception, Error
	{
		...
		try {
			...
			RMAApp_ExtentReports="SuiteResults_"+RMAApp_BrowserName + "_"+ RMAApp_ExtentReportsHtmlName + "_"+ dtmCurrentDateInfo + "_" + dtmCurrentTimeInfo +  ".html"; //ExtentReport Name
			RMAApp_ExtentReportsLocation = ExcelData.RMA_ExcelDataRead_Utility(0,IntRowNumber,1) + "//"+RMAApp_ExtentReports; //ExtentReport Location Fetched From DataSheet RMA_Utilities
	...
	public static void RMA_ExtentFailureReport(String FinalErrorMessage, String TestCaseName, String StrScreenShotTCName,int logval)throws Exception, Error
	{
		try {
			...
			RMA_GenericUsages_Utility.RMA_WindowsMessageHandler_Utility(driver, "Yes");		
			strFailedTestCaseDetails+=i+". "+TestCaseName+Keys.ENTER;
			i++;
			//System.out.println(strFailedTestCaseDetails+"strFailedTestCaseDetails");
			if (logval ==0){
				parentlogger.log(LogStatus.FAIL, "Following Error Occurred  :::  <span style='color:red'>" + FinalErrorMessage+ "</script></span>" +"While Executing Test Case" +" "+ StrScreenShotTCName + " " +  "And Hence The Test Case Is A Fail");
			...
			{
				logger.log(LogStatus.FAIL, "Following Error Occurred  :::  <span style='color:red'>" + FinalErrorMessage+ "</script></span>" +"While Executing Test Case" +" "+ StrScreenShotTCName + " " +  "And Hence The Test Case Is A Fail");
			...
			if(!UtilityMenu.RMAUtilityurl.equals(null)){
				driver.quit();
				UtilityMenu.frame.repaint();
				UtilityMenu.frame.setVisible(true);
				JOptionPane.showMessageDialog(UtilityMenu.frame, "Desired Utilty Failed Due to Unexpected Error! Please Refer Extent Report for More Details.");
				UtilityMenu.frame.requestFocus();				
				UtilityMenu.frame.repaint();
				UtilityMenu.frame.toFront();			
			}
