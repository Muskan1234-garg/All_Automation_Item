...
import rmaseleniumPOM_UX.RMA_Selenium_POM_Globalization;
...
public class RMA_Verification_Utility extends rmaseleniumtestscripts.RMA_TC_BaseTest{
	...
	//Revision			: 0.1 - RenuVerma-09-06-2018// UX screen has no loading bar     
	//=============================================================================================================================================================================================================================================
	public static void RMA_VerifyLoadingBarDisappered( int logval) throws Exception, Error
	{
		String CurrentFrameId;

		RMA_VerifyUXSpinnerDisappered(logval);
		/*try {
	...
	//Modify By			: 0.0 - RenuVerma-10-26-2018 As Utility Screen does not have the previous DOM
	//==============================================================================================================================================================================================================================================================================================================================================================================================
	public static void RMA_VerifyUXSpinnerDisappered( int logval) throws Exception, Error
	{
		...
		try {
			...
			try {
				if(driver.findElement(By.xpath("//div[@ng-show='commonLoader']"))!=null)
					WebDriverWait.until(ExpectedConditions.attributeContains(By.xpath("//div[@ng-show='commonLoader']"),"class","ng-hide")) ;// Wait For Load Image To Be Disappear// Wait to disapper the Spinner
			} catch (Exception e1) {
				WebDriverWait.until(ExpectedConditions.presenceOfElementLocated(By.xpath("//div[@class='CustompleaseWaitFrame']")));// To handle the utility Screen
			}
			if (logval ==0)
			{
	...
	public static void RMA_VerifyUXGrid_Utility(String SearchedText, String UXGridDescription,  String[] ExpectedColHeadersToBeVerified, Object[] ExpectedValuesToBeVerified,int logval ) throws Exception,Error		
	{
			...
			if(ExpectedColHeadersToBeVerified.length==ExpectedValuesToBeVerified.length)// Check the length of the given Column header should equal to Given Column Values
			{
				...
				for(int k=0;k<ExpectedColHeadersToBeVerified.length;k++)// Outer loop for given column size and find expected value column no
				{
					...
					if(FoundCol==-1){
						if (logval == 0)
						{
							parentlogger.log(LogStatus.FAIL, "Expected Column Header : " +" " +color.RMA_ChangeColor_Utility(ExpectedColHeadersToBeVerified[k],2) +" " +"Is Not Found In " +"SearchedTextActual is"+SearchedTextActual+" " + color.RMA_ChangeColor_Utility(UXGridDescription,4) +" " +color.RMA_ChangeColor_Utility(", Hence Verification Can Not Be Done",1));
						}
						else
						{
							logger.log(LogStatus.FAIL, "Expected Column Header : " +" " +color.RMA_ChangeColor_Utility(ExpectedColHeadersToBeVerified[k],2) +" " +"Is Not Found In " +"SearchedTextActual is"+SearchedTextActual+" " + color.RMA_ChangeColor_Utility(UXGridDescription,4) +" " +color.RMA_ChangeColor_Utility(", Hence Verification Can Not Be Done",1));
						}
					...
					{			
						if (logval == 0)
						{
							System.out.println("ExpectedValuesToBeVerified[k] is "+ExpectedValuesToBeVerified[k]);
		...
		catch(Exception|Error e)
		{
			if (logval == 0)
			{
				parentlogger.log(LogStatus.FAIL, "Verification Can Not Be Done Due to error "+color.RMA_ChangeColor_Utility(e.toString(),1));
			}
			else
			{
				logger.log(LogStatus.FAIL, "Verification Can Not Be Done Due to error "+color.RMA_ChangeColor_Utility(e.toString(),1));
			}
				...
				outerloop:
					for(int i = 0; i< UX_row.size(); i++) // run the loop to which DOM row matching with the given expected SearchedText
					{
						...
						for(int j=0; j< UX_column.size(); j++)// run the loop to which DOM column to the corresponding row matching with the given expected SearchedText
						{
							//SearchedTextActual = UX_column.get(j).getText().trim();//Get each search text value
							SearchedTextActual =((JavascriptExecutor) driver).executeScript("return arguments[0].innerText;", UX_column.get(j) ).toString();
							System.out.println("SearchedTextActual"+SearchedTextActual);
							if (SearchedTextActual.trim().equalsIgnoreCase(SearchedText.trim()))// compare DOM column to the corresponding row with the given expected SearchedText
							{
								FoundRow = i;// Store row 
								break outerloop;
	...
	/*public static void RMA_TableSingleTextVerify_Utility(WebElement Element, String SearchText, int col1, String ExpVal1, String TableName, int logval) throws Exception, Error
	{
		try {
			...
			try {
				table_rows = Element.findElements(By.tagName("tr"));
			} catch (Exception e) {
				throw new Exception("Table Element Is Not Found And Error Is"+e.getMessage());

			}
			IntTable_RowsCount = table_rows.size(); //RowCount Is Stored In Variable IntTable_RowsCount
			//System.out.println("IntTable_RowsCount"+IntTable_RowsCount);
			breakloop:
				for (int row=0; row<IntTable_RowsCount; row++){ //Loop Is Iterated Through All The Rows Of The Table
					...
					//System.out.println("IntTable_ColumnsCount"+IntTable_ColumnsCount);
					for (int column=0; column<IntTable_ColumnsCount; column++){//Loop Is Iterated Through All The Columns Of The Row
						...
						//System.out.println("IntTable_CellText"+IntTable_CellText+"column: "+column+" SearchText: "+SearchText);
						if (SearchText.equalsIgnoreCase(IntTable_CellText)){
							StrActualVal1 = table_rows_columns.get(col1).getText();
							//System.out.println("StrActualVal1"+StrActualVal1);
							if (StrActualVal1.equalsIgnoreCase(ExpVal1)){
								if (logval==0)
								{
									...
									parentlogger.log(LogStatus.INFO, logger.addScreenCapture(RMA_ScreenCapture_Utility.RMA_ScreenShotCapture_Utility(driver,"Table Verification Done", "Table Verification Done")));

								}
								else
								{
									logger.log(LogStatus.PASS, "For Table::" + " " + color.RMA_ChangeColor_Utility(TableName, 4) + " "  + "And Searched Text Value::" + " " + color.RMA_ChangeColor_Utility(SearchText, 4)+ " " + "Expected Value::" + " " + color.RMA_ChangeColor_Utility(ExpVal1, 3) + " " +  "Is Equal To Actual Value" + " " +  color.RMA_ChangeColor_Utility(StrActualVal1, 2) + " " + "And" +" " + "Hence Table Value Verification Is Successful" );
									logger.log(LogStatus.INFO, logger.addScreenCapture(RMA_ScreenCapture_Utility.RMA_ScreenShotCapture_Utility(driver, "Table Verification Done","Table Verification Done")));
								...
								if (logval==0)
								{
									...
									parentlogger.log(LogStatus.INFO, logger.addScreenCapture(RMA_ScreenCapture_Utility.RMA_ScreenShotCapture_Utility(driver,"Table Verification Failed", "Table Verification Failed")));
								}
								else
								{
									...
									logger.log(LogStatus.INFO, logger.addScreenCapture(RMA_ScreenCapture_Utility.RMA_ScreenShotCapture_Utility(driver,"Table Verification Failed", "Table Verification Failed")));
			...
			if(StrActualVal1.equalsIgnoreCase(ExpVal1))
				Assert.assertTrue(StrActualVal1.equalsIgnoreCase(ExpVal1),StrActualVal1+"Is Present In Table");
				else 
				Assert.assertTrue(StrActualVal1.equalsIgnoreCase(ExpVal1),StrActualVal1+"Is Not Present In Table"); 
			...
			throw new Exception("Single Text Verification on Table "+TableName+" Is Failed As "+e.getMessage());
		}
	}*/
	...
	public static void RMA_TableSingleTextVerify_Utility(WebElement Element, String SearchText, int col1, String ExpVal1, String TableName, int logval) throws Exception, Error
	{
		try {
			...
			if(StrActualVal1.equalsIgnoreCase(ExpVal1))
				...
				Assert.assertFalse(StrActualVal1.equalsIgnoreCase(ExpVal1),StrActualVal1+"Is Not Present In Table");

		} catch (Exception|Error e) {
			if (logval==0)
			{
				parentlogger.log(LogStatus.INFO, "For Table::" + " " + color.RMA_ChangeColor_Utility(TableName, 4) + " " + "Value Verification Is Not Successful Due To The Following Reason" + color.RMA_ChangeColor_Utility(e.getMessage(),1) );  
			}
			else
			{
				logger.log(LogStatus.INFO, "For Table::" + " " + color.RMA_ChangeColor_Utility(TableName, 4) + " " + "Value Verification Is Not Successful Due To The Following Reason" + color.RMA_ChangeColor_Utility(e.getMessage(),1) ); 
			}
	...
	/*	public static void RMA_VerifyRowDataForTwoColTablePartially(WebElement Element,String SrchedText1, String SrchedText2, String ExpectedValue, int ExpectedCol, String ObjDes, int logval ) throws Exception,Error
	{
		try
		{
			...
			try {		
				...
				throw new Exception("Table row with corresponding ID is not Found: "+e.getMessage());
			}
			loop:
				...
				{	
					...
					try {
						TableCol = TableRows.get(i).findElements(By.tagName("td"));
					} catch (Exception e) {
						throw new Exception("Table column with corresponding ID is not Found: "+e.getMessage());
					}				
	...
	public static void RMA_VerifyRowDataForTwoColTablePartially(WebElement Element,String SrchedText1, String SrchedText2, String ExpectedValue, int ExpectedCol, String ObjDes, int logval ) throws Exception,Error
	{
		try
		{
				...
				{     
					...
					{ 
						...
						else 
						{
							...
							if (logval == 0)
							{
								parentlogger.log(LogStatus.INFO,color.RMA_ChangeColor_Utility(ErrorMsg1, 1) );
								Assert.fail(ErrorMsg1);
							}

							else
							{
								logger.log(LogStatus.INFO,color.RMA_ChangeColor_Utility(ErrorMsg1, 1) );
								Assert.fail(ErrorMsg1);
							}
						}                       
						break loop;
			...
			if (Found == "False")
			{
				...
				if (logval == 0)
				{
					parentlogger.log(LogStatus.INFO, color.RMA_ChangeColor_Utility(Errormsg, 1) );
					Assert.fail(Errormsg);
				}
				else
				{
					logger.log(LogStatus.INFO, color.RMA_ChangeColor_Utility(Errormsg, 1) );
					Assert.fail(Errormsg);
				}
			}
	...
	public static void RMA_VerifyUXMessage( String Message,boolean IsCloseRequired,String ObjPageDescription, int logval) throws Exception, Error
	{
		...
		try {
			...
			if(IsCloseRequired)
				RMA_Navigation_Utility.RMA_ObjectJavaScriptClick(RMA_Selenium_POM_Home.RMAApp_GenericBtn_ErrorToastMsgClose(driver), "Error/Warning Toast Close Button On "+ ObjPageDescription, logval);			
			Thread.sleep(3000);
		} catch (Exception|Error e) {
			throw (e);
		}
	...
	//Revision			: 0.1 - RenuVerma-09-15-2017   Updated functionality to make is more generic from all scenario of pdf 
	// ==========================================================================================================================================================================================================================================================
	//MK Changes Starts
		public static String PDFReaderContentVerification_Utility(String PDFPassword,Object[] ContentToBeVerified, String CheckContentAvailableOrNot, int Intlogval) throws Exception, Error{
			...
			try {
				...
				if(CheckContentAvailableOrNot.equalsIgnoreCase("Yes")){
				...
				else if(CheckContentAvailableOrNot.equalsIgnoreCase("No")){
					//PDF Verification Started Here
					for(int i=0;i<ContentToBeVerified.length;i++){
						RMA_NegativeVerification_Utility.RMA_NoPartialTextVerification(ContentToBeVerified[i].toString(),parsedText,ContentToBeVerified[i].toString()+" Value Verification", Intlogval);	
					}}
				else if(CheckContentAvailableOrNot.equalsIgnoreCase("YesNo")){ //Workaround to verify one control number is available and another is not available For Test Case RMA_Reg_Super_PrintCheckLimit_TC_003
					//PDF Verification Started Here
					for(int i=0;i<ContentToBeVerified.length;i++){
						RMA_Verification_Utility.RMA_PartialTextVerification(ContentToBeVerified[0].toString(),parsedText,ContentToBeVerified[0].toString()+" Value Verification", Intlogval);	
						RMA_NegativeVerification_Utility.RMA_NoPartialTextVerification(ContentToBeVerified[1].toString(),parsedText,ContentToBeVerified[1].toString()+" Value Verification", Intlogval);	
					}}
				else if(CheckContentAvailableOrNot.equalsIgnoreCase("NA")){
					//PDF Verification Not Required
					}
				else{
					parentlogger.log(LogStatus.FAIL, "Wrong Choice");
				}
	...
	//====================================================================================================
	//FunctionName 		: RMA_VerifyNotExistSingle_MultipleWebListData
	//Description  		: Enables The User To Verify Single/Multiple Data Values In The WebList 
	//Input Parameter 	: Element, WebListTextToBeVerified: Text/List To Be Verified, ObjDesc, logval 
	//Revision			: 0.0 - ShrutiShruti-03-20-2018                               
	//====================================================================================================
	public static void RMA_VerifyNotExistSingle_MultipleWebListData(WebElement Element,String WebListTextToBeVerified, String ObjDesc, int logval ) throws Exception,Error		
	{
		String[] TextToBeVerified= WebListTextToBeVerified.split(";");// User Input should be separated by ';'
		...
		{	
			...
			for (int i=0; i<TextToBeVerified.length; i++) // Outer loop runs till the given Text Value count
			{  
				boolean notfound=true;// initialize match value as false for each TextToBeVerified array value
				innerloop:
					for(WebElement we:options)  // Loop runs till the WebList Options count
					{
				...
				if(notfound){// Check match value for false condition
					if (logval == 0)
					{
						parentlogger.log(LogStatus.PASS, color.RMA_ChangeColor_Utility("Corresponding List Element "+ color.RMA_ChangeColor_Utility(TextToBeVerified[i],2)+" Is Not Available In " +color.RMA_ChangeColor_Utility(ObjDesc,4)+" Hence Can Continue Testing.", 3) );
					}
					else
					{
						logger.log(LogStatus.PASS, color.RMA_ChangeColor_Utility("Corresponding List Element "+ color.RMA_ChangeColor_Utility(TextToBeVerified[i],2)+" Is Not Available In " +color.RMA_ChangeColor_Utility(ObjDesc,4)+" Hence Can Continue Testing.", 3) );
					}
					notfound=true;
	...
	//============================================================================================
	//FunctionName 		: RMA_CheckTextInTableRow
	//Description  		: To Verify The Text In Particular Row of  A Table Based On The User Provided Search Value, 
	//Input Parameter 	: SearchedText: Text To Be Searched, TableCol : Collection of WebElement With Tag td
	//Revision			: 0.0 - DebasmitaPati-08-28-2018                                 
	//============================================================================================
	private static Boolean RMA_CheckTextInTableRow(List<WebElement> TableCol , String SearchedText)
	{
		Boolean TextFound = true;
		Boolean TextNotFound = false;
		SearchedText = SearchedText.trim();
		...
		for (int j = 0; j<TableCol.size(); j++)
		{
			String CellTextValue =  TableCol.get(j).getText();
			CellTextValue = CellTextValue.toUpperCase();
			if (CellTextValue.trim().equalsIgnoreCase(SearchedText.trim()))
			{
				return TextFound;
			}
		}
	...
	//=====================================================================================================================================================================================================================================================
	//FunctionName 		: RMA_VerifyTableTextOnTwoColValues_Utility
	//Description  		: Enables The User To Verify Expected Values In Different Columns, After Finding An Unique Row Based On The User Provided Two Search Values
	//Input Parameter 	: Element: Table Element,SearchedText1: Text1 To Be Searched From Table, SearchedText2: Text2 To Be Searched From Table, TableName:TableName,ExpectedColHeadersToBeVerified: Expected Col Headers To Be Verified separated by ';'(e.g. Reserve Type, Balance on Financial Reserve Grid),ExpectedValuesToBeVerified:Expected Values To Be Verified 
	//Revision			: 0.0 - DebasmitaPati-07-30-2018                               
	//=====================================================================================================================================================================================================================================================
	public static void RMA_VerifyTableTextOnTwoColValues_Utility(WebElement Element,String SearchedText1,String SearchedText2, String TableName,  String[] ExpectedColHeadersToBeVerified, Object[] ExpectedValuesToBeVerified,int logval ) throws Exception,Error		
	{
		int FoundRow;// To find row id of search text
		String ActualText;
		String SearchedTextActual = null;//initialize 
		int FoundCol;// to find column index of the given expected value
		try
		{					
			if(ExpectedColHeadersToBeVerified.length==ExpectedValuesToBeVerified.length)// Check the length of the given Column header should equal to Given Column Values
			{
				...
				List<WebElement> TableRows = Element.findElements(By.tagName("tr")); //All The Row Elements Of The Table Are Fetched In A List
				for(int i = 0; i< TableRows.size(); i++) //Loop Is Iterated Through All The Rows Of The Table
				{
					List<WebElement> TableCol = TableRows.get(i).findElements(By.tagName("td"));  //All The Column Elements Of The Table Are Fetched In A List	
					if (RMA_CheckTextInTableRow(TableCol,  SearchedText1) )// compare column to the corresponding row with the given expected SearchedText1 And SearchedText2
					{
						FoundRow = i;// Store row 
						List<WebElement> TableColfound=TableRows.get(FoundRow).findElements(By.tagName("td"));
						if (RMA_CheckTextInTableRow(TableColfound, SearchedText2)){							
							break;
						}
						else continue;
					}
				...
				for(int k=0;k<ExpectedColHeadersToBeVerified.length;k++)// Outer loop for given column size and find expected value column no
				{
					...
					List<WebElement> TableHeader = Element.findElements(By.tagName("th")); //All The Header Elements Of The Table Are Fetched In A List
					for(int j=0; j< TableHeader.size(); j++) //Loop Is Iterated Through All The Headers Of The Table
					{
						SearchedTextActual =((JavascriptExecutor) driver).executeScript("return arguments[0].innerText;", TableHeader.get(j) ).toString();
						if (SearchedTextActual.trim().equalsIgnoreCase(ExpectedColHeadersToBeVerified[k].trim()))// If DOM column header is equal to given expected column header
						{
							FoundCol = j;// store value in FoundCol
							break;
					...
					if(FoundCol==-1){
						if (logval == 0)
						{
							parentlogger.log(LogStatus.FAIL, "Expected Column Header : " +" " +color.RMA_ChangeColor_Utility(ExpectedColHeadersToBeVerified[k],2) +" " +"Is Not Found In " +" " + color.RMA_ChangeColor_Utility(TableName,4) +" " +color.RMA_ChangeColor_Utility(", Hence Verification Can Not Be Done",1));
						}
						else
						{
							logger.log(LogStatus.FAIL, "Expected Column Header : " +" " +color.RMA_ChangeColor_Utility(ExpectedColHeadersToBeVerified[k],2) +" " +"Is Not Found In " +" " + color.RMA_ChangeColor_Utility(TableName,4) +" " +color.RMA_ChangeColor_Utility(", Hence Verification Can Not Be Done",1));
						}
					...
					if (FoundRow != -1)// if given searched text found
					{
						List<WebElement> TableCol = TableRows.get(FoundRow).findElements(By.tagName("td"));
						ActualText =((JavascriptExecutor) driver).executeScript("return arguments[0].innerText;", TableCol.get(FoundCol)).toString();

						if (ActualText.trim().equalsIgnoreCase(String.valueOf(ExpectedValuesToBeVerified[k]).trim()))// if found column text value is equal to expected value
						{
							if (logval == 0)
							{
								parentlogger.log(LogStatus.PASS, "Expected Values : " +" " +color.RMA_ChangeColor_Utility(String.valueOf(ExpectedValuesToBeVerified[k]),3) +" Are Matched With Actual Values :"+" " +color.RMA_ChangeColor_Utility(ActualText,2) +" , In Column :" +" " +color.RMA_ChangeColor_Utility(ExpectedColHeadersToBeVerified[k],4) +" " +"And " +" " +" " +"Respectively, Corresponding To Indentifier SearchedText1 :" +" "  +color.RMA_ChangeColor_Utility(SearchedText1,2)+" " +"And SearchedText2 :" +" " +color.RMA_ChangeColor_Utility(SearchedText2,2) +", In"+" " +color.RMA_ChangeColor_Utility(TableName,4) +" "+", And Hence Verification Is Successfull");
							}
							else
							{
								logger.log(LogStatus.PASS, "Expected Values : " +" " +color.RMA_ChangeColor_Utility(String.valueOf(ExpectedValuesToBeVerified[k]),3) +" Are Matched With Actual Values :"+" " +color.RMA_ChangeColor_Utility(ActualText,2) +", In Column :" +" " +color.RMA_ChangeColor_Utility(ExpectedColHeadersToBeVerified[k],4) +" " +"And " +" " +" " +"Respectively, Corresponding To Indentifier SearchedText1 :" +" "  +color.RMA_ChangeColor_Utility(SearchedText1,2)+" " +"And SearchedText2 :" +" " +color.RMA_ChangeColor_Utility(SearchedText2,2) +", In"+" " +color.RMA_ChangeColor_Utility(TableName,4) +" "+", And Hence Verification Is Successfull");
						...
						else// if found column text value is not equal to expected value
						{
							if (logval == 0)
							{
								parentlogger.log(LogStatus.FAIL, "Expected Values : " +" " +color.RMA_ChangeColor_Utility(String.valueOf(ExpectedValuesToBeVerified[k]),3) +" Are Not Matched With Actual Values :"+" " +color.RMA_ChangeColor_Utility(ActualText,2) +" , In Column :" +" " +color.RMA_ChangeColor_Utility(ExpectedColHeadersToBeVerified[k],4) +" " +"And " +" "  +" " +"Respectively, Corresponding To Indentifier SearchedText1 :" +" "  +color.RMA_ChangeColor_Utility(SearchedText1,2)+" " +"And SearchedText2 :" +" " +color.RMA_ChangeColor_Utility(SearchedText2,2) +", In"+" " +color.RMA_ChangeColor_Utility(TableName,4) +" "+color.RMA_ChangeColor_Utility(", And Hence Verification Is Not Successfull",1));
							}
							else
							{ 
								logger.log(LogStatus.FAIL, "Expected Values : " +" " +color.RMA_ChangeColor_Utility(String.valueOf(ExpectedValuesToBeVerified[k]),3) +" Are Not Matched With Actual Values :"+" " +color.RMA_ChangeColor_Utility(ActualText,2) +" , In Column :" +" " +ExpectedColHeadersToBeVerified[k] +" " +"And " +" "  +" " +"Respectively, Corresponding To Indentifier SearchedText1 :" +" "  +color.RMA_ChangeColor_Utility(SearchedText1,2)+" " +"And SearchedText2 :" +" " +color.RMA_ChangeColor_Utility(SearchedText2,2) +", In"+" " +color.RMA_ChangeColor_Utility(TableName,4) +" "+color.RMA_ChangeColor_Utility(", And Hence Verification Is Not Successfull",1));
							}
							Assert.assertTrue(ActualText.equalsIgnoreCase(ExpectedValuesToBeVerified[k].toString().trim()));
						}	
					}
					...
					{			
						if (logval == 0)
						{
							parentlogger.log(LogStatus.FAIL, "SearchedText1 : " +" " +color.RMA_ChangeColor_Utility(SearchedText1,2)+" " +"And SearchedText2 :" +" " +color.RMA_ChangeColor_Utility(SearchedText2,2) +" " +"Is Not Found In " +" " + color.RMA_ChangeColor_Utility(TableName,4) +" " +color.RMA_ChangeColor_Utility(", Hence Verification Can Not Be Done",1));
						}
						else
						{
							logger.log(LogStatus.FAIL, "SearchedText1 : " +" " +color.RMA_ChangeColor_Utility(SearchedText1,2)+" " +"And SearchedText2 :" +" " +color.RMA_ChangeColor_Utility(SearchedText2,2) +" " +"Is Not Found In " +" " + color.RMA_ChangeColor_Utility(TableName,4) +" " +color.RMA_ChangeColor_Utility(", Hence Verification Can Not Be Done",1));
						}
						Assert.assertTrue(FoundRow!= -1);
					}
				}
				if (logval == 0)//Final screen shot
				{
					parentlogger.log(LogStatus.INFO, parentlogger.addScreenCapture(RMA_ScreenCapture_Utility.RMA_ScreenShotCapture_Utility(driver, "Table Verfication", TableName)));
				}
				else
				{ 
					logger.log(LogStatus.INFO, logger.addScreenCapture(RMA_ScreenCapture_Utility.RMA_ScreenShotCapture_Utility(driver, "Table Verfication", TableName)));
				}
			}
			else{  //if  Check the length of the given Column header is not equal to Given Column Values
				if (logval == 0)
	...
	//============================================================================================
	//FunctionName 		: RMA_VerifyTextInUXGridRowPartially
	//Description  		: To Verify The Text In Particular Row of A UX Grid Based On The User Provided Search Value
	//Input Parameter 	: SearchedText: Text To Be Searched, UX_column : CSS Selector of the Corresponding Column of the  Search Text row Of The UX grid
	//Revision			: 0.0 - DebasmitaPati-07-27-2018                                 
	//============================================================================================
	private static Boolean RMA_VerifyTextInUXGridRowPartially(List<WebElement> UX_column , String SearchedText)
	{
		Boolean TextFound = true;
		Boolean TextNotFound = false;
		String SearchedTextActual = null;//initialize 
		...
		{
			SearchedTextActual =((JavascriptExecutor) driver).executeScript("return arguments[0].innerText;", UX_column.get(j) ).toString();
			if (SearchedTextActual.trim().equalsIgnoreCase(SearchedText.trim()))
			{
				return TextFound;
			}
		}
	...
	//=====================================================================================================================================================================================================================================================
	//FunctionName 		: RMA_VerifyUXGridOnTwoColValues_Utility
	//Description  		: Enables The User To Verify Expected Values In Different Columns, After Finding An Unique Row Based On The User Provided Two Search Values
	//Input Parameter 	: SearchedText1: Text1 To Be Searched From UXGrid, SearchedText2: Text2 To Be Searched From UXGrid, UXGridDescription:GridDescription,ExpectedColHeadersToBeVerified: Expected Col Headers To Be Verified separated by ';'(e.g. Reserve Type, Balance on Financial Reserve Grid),ExpectedValuesToBeVerified:Expected Values To Be Verified (e.g. Reserve Type value, Balance value on Financial Reserve Grid),logval
	//Revision			: 0.0 - DebasmitaPati-07-25-2018                               
	//=====================================================================================================================================================================================================================================================
	public static void RMA_VerifyUXGridOnTwoColValues_Utility(String SearchedText1,String SearchedText2, String UXGridDescription,  String[] ExpectedColHeadersToBeVerified, Object[] ExpectedValuesToBeVerified,int logval ) throws Exception,Error		
	{
		int FoundRow;// To find row id of search text
		String ActualText;
		String SearchedTextActual = null;//initialize 
		...
		{					
			if(ExpectedColHeadersToBeVerified.length==ExpectedValuesToBeVerified.length)// Check the length of the given Column header should equal to Given Column Values
			{
				...
				for(int i = 0; i< UX_row.size(); i++) // run the loop to which DOM row matching with the given expected SearchedText
				{
					WebElement Element =UX_row.get(i).findElement(By.xpath(".//*[@class='ng-isolate-scope']"));// Xpath of the Row value The UX grid
					List<WebElement> UX_column = Element.findElements(By.cssSelector("div[class^='ui-grid-cell ng-scope ui-grid-coluiGrid']"));// CSS Selector of the Corresponding Column of the  Search Text row Of The UX grid
					if (RMA_VerifyTextInUXGridRowPartially(UX_column,  SearchedText1) && RMA_VerifyTextInUXGridRowPartially(UX_column, SearchedText2))// compare DOM column to the corresponding row with the given expected SearchedText1 And SearchedText2
					{
						FoundRow = i;// Store row 
				...
				for(int k=0;k<ExpectedColHeadersToBeVerified.length;k++)// Outer loop for given column size and find expected value column no
				{
					...
					if (FoundRow != -1)// if given searched text found
					{
						...
						if (ActualText.trim().equalsIgnoreCase(String.valueOf(ExpectedValuesToBeVerified[k]).trim()))// if found column text value is equal to expected value
						{
							if (logval == 0)
							{
								parentlogger.log(LogStatus.PASS, "Expected Values : " +" " +color.RMA_ChangeColor_Utility(String.valueOf(ExpectedValuesToBeVerified[k]),3) +" Are Matched With Actual Values :"+" " +color.RMA_ChangeColor_Utility(ActualText,2) +" , In Column :" +" " +color.RMA_ChangeColor_Utility(ExpectedColHeadersToBeVerified[k],4) +" " +"And " +" " +" " +"Respectively, Corresponding To Indentifier SearchedText1 :" +" "  +color.RMA_ChangeColor_Utility(SearchedText1,2)+" " +"And SearchedText2 :" +" " +color.RMA_ChangeColor_Utility(SearchedText2,2) +", In"+" " +color.RMA_ChangeColor_Utility(UXGridDescription,4) +" "+", And Hence Verification Is Successfull");
							}
							else
							{
								logger.log(LogStatus.PASS, "Expected Values : " +" " +color.RMA_ChangeColor_Utility(String.valueOf(ExpectedValuesToBeVerified[k]),3) +" Are Matched With Actual Values :"+" " +color.RMA_ChangeColor_Utility(ActualText,2) +", In Column :" +" " +color.RMA_ChangeColor_Utility(ExpectedColHeadersToBeVerified[k],4) +" " +"And " +" " +" " +"Respectively, Corresponding To Indentifier SearchedText1 :" +" "  +color.RMA_ChangeColor_Utility(SearchedText1,2)+" " +"And SearchedText2 :" +" " +color.RMA_ChangeColor_Utility(SearchedText2,2) +", In"+" " +color.RMA_ChangeColor_Utility(UXGridDescription,4) +" "+", And Hence Verification Is Successfull");
						...
						else// if found column text value is not equal to expected value
						{
							if (logval == 0)
							{
								parentlogger.log(LogStatus.FAIL, "Expected Values : " +" " +color.RMA_ChangeColor_Utility(String.valueOf(ExpectedValuesToBeVerified[k]),3) +" Are Not Matched With Actual Values :"+" " +color.RMA_ChangeColor_Utility(ActualText,2) +" , In Column :" +" " +color.RMA_ChangeColor_Utility(ExpectedColHeadersToBeVerified[k],4) +" " +"And " +" "  +" " +"Respectively, Corresponding To Indentifier SearchedText1 :" +" "  +color.RMA_ChangeColor_Utility(SearchedText1,2)+" " +"And SearchedText2 :" +" " +color.RMA_ChangeColor_Utility(SearchedText2,2) +", In"+" " +color.RMA_ChangeColor_Utility(UXGridDescription,4) +" "+color.RMA_ChangeColor_Utility(", And Hence Verification Is Not Successfull",1));
							}
							else
							{ 
								logger.log(LogStatus.FAIL, "Expected Values : " +" " +color.RMA_ChangeColor_Utility(String.valueOf(ExpectedValuesToBeVerified[k]),3) +" Are Not Matched With Actual Values :"+" " +color.RMA_ChangeColor_Utility(ActualText,2) +" , In Column :" +" " +ExpectedColHeadersToBeVerified[k] +" " +"And " +" "  +" " +"Respectively, Corresponding To Indentifier SearchedText1 :" +" "  +color.RMA_ChangeColor_Utility(SearchedText1,2)+" " +"And SearchedText2 :" +" " +color.RMA_ChangeColor_Utility(SearchedText2,2) +", In"+" " +color.RMA_ChangeColor_Utility(UXGridDescription,4) +" "+color.RMA_ChangeColor_Utility(", And Hence Verification Is Not Successfull",1));
							}
							Assert.assertTrue(ActualText.equalsIgnoreCase(ExpectedValuesToBeVerified[k].toString().trim()));
						}	
					}
					...
					{			
						if (logval == 0)
						{
							parentlogger.log(LogStatus.FAIL, "SearchedText1 : " +" " +color.RMA_ChangeColor_Utility(SearchedText1,2)+" " +"And SearchedText2 :" +" " +color.RMA_ChangeColor_Utility(SearchedText2,2) +" " +"Is Not Found In " +" " + color.RMA_ChangeColor_Utility(UXGridDescription,4) +" " +color.RMA_ChangeColor_Utility(", Hence Verification Can Not Be Done",1));
						}
						else
						{
							logger.log(LogStatus.FAIL, "SearchedText1 : " +" " +color.RMA_ChangeColor_Utility(SearchedText1,2)+" " +"And SearchedText2 :" +" " +color.RMA_ChangeColor_Utility(SearchedText2,2) +" " +"Is Not Found In " +" " + color.RMA_ChangeColor_Utility(UXGridDescription,4) +" " +color.RMA_ChangeColor_Utility(", Hence Verification Can Not Be Done",1));
						}
						Assert.assertTrue(FoundRow!= -1);
					}
				}
	...
	//====================================================================================================================================================================================================================================================================================================================================================================================================================================================================
	//FunctionName 		: RMA_ElementWebListGetValue_Utility
	//Description  		: To Get The Selected Value In A Drop Down Element
	//Input Parameter 	: Element Of Type WebElement: Element From Which The Value Is To Be Fetched
	//Revision			: 0.0 - DebasmitaPati-08-21-2018                               
	//=======================================================================================================================================================================================================================================================================================================================================================================================================================================================================
	public static String RMA_ElementWebListGetValue_Utility(WebElement Element) throws Exception, Error
	{
	...
	//=====================================================================================================================================================================================================================================================
	//FunctionName 		: RMA_VerifyExchangeRate_Utility
	//Description  		: Enables The User To Verify If Exchange Rate Is Present For Different Currency and Add The same If Record Is Not Present On Exchange Rate Setup Screen and Update The Same If Exchange Rate Is Present For The Given Currencies 
	//Input Parameter 	: Source Currency, Destination Currency And Current Exchange Rate Passed Through XML
	//Revision			: 0.0 - ParulSharma-11-30-2019                               
	//=====================================================================================================================================================================================================================================================
	public static double RMA_VerifyExchangeRate_Utility(String Source, String Destination,double Rate1,int logval ) throws Exception,Error		
	{
		int found_value=0; // To Know If Expected Exchange Rate Is Present On The Screen Or Not 
		String SearchedActualRate = String.valueOf(Rate1) ; // To Know actual Exchange Rate Added/Present On The Given Screen
		String Handle = driver.getWindowHandle();//To Know Which Browser Window Is Opened when The Given Function Was Called
		String CurrentRowValue;
		String CurrentColValue;
		String destination_Source_Excahnge_Rate =null;
		try
		{					
			if (logval == 0)
			{
				parentlogger.log(LogStatus.INFO, "Enables The User To Verify If Exchange Rate Is Present For Different Currency and Add The same If Record Is Not Present On Exchange Rate Setup Screen");
			}else{	
				logger = reports.startTest("<span id='ExchangeRateSetup'>Verify/Add Rates On Exchange Rate Setup</span>","Enables The User To Verify If Exchange Rate Is Present For Different Currency and Add The same If Record Is Not Present On Exchange Rate Setup Screen");
			}
			int lastrowno =0;
			WebElement Table_body = RMA_Selenium_POM_Globalization.RMAApp_ExchangeRateSetup_ExistingRecord_Table(driver);
			List<WebElement>TotalRowsList =Table_body.findElements(By.tagName("tr"));
			for(int i=0;i<=TotalRowsList.size()-2;i++)// Loop For Finding Whether Exchange Rate for Exists For Source And Destination Currency Or Not And If Exists Then Update The Same
			{
				CurrentRowValue =driver.findElement(By.id("MultiCurrencySetupGrid_gvData_ctl07_"+i+"")).getText(); // For Extracting Value Of 1St Column Of The Given Row
				if(CurrentRowValue.equals(Source))
				{ //If Condition For Finding whether Given Column Value of Current Row Is Same AS Source Or Not

					CurrentColValue = driver.findElement(By.id("MultiCurrencySetupGrid_gvData_ctl09_"+i+"")).getText();
					if(CurrentColValue.equals(Destination))
					{   //If Condition For Finding whether Given Column Value Is Same AS Destination Or Not
						destination_Source_Excahnge_Rate = driver.findElement(By.id("MultiCurrencySetupGrid_gvData_ctl11_"+i+"")).getText(); //For Extracting Value Of 2nd Column Of The Given Row
						RMA_Navigation_Utility.RMA_ObjectClick(RMA_Selenium_POM_Globalization.RMAApp_Globalization_Option_Button(driver,i), "option button clicked on exchange rate record",logval);
						RMA_Navigation_Utility.RMA_ObjectClick(RMA_Selenium_POM_Globalization.RMAApp_ExchangeRateSetup_Button_Edit(driver), "Edit button clicked on exchange rate record",logval);

						RMA_GenericUsages_Utility.RMA_StaticWait(20, logval,"Wait Is Added For Frame To Get Loaded");
						RMA_GenericUsages_Utility.RMA_WindowSwitchingBasedOnTitle("Add/Modify Currency");

						RMA_Input_Utility.RMA_SetTextValueandTabOut_Utility(RMA_Selenium_POM_Globalization.RMAApp_ExchangeRateSetup_ExchangeRate_InputBox(driver) , "Currency Type field on Globalization tab", String.valueOf(Rate1), logval);
						RMA_Navigation_Utility.RMA_ObjectActionsClick(RMA_Selenium_POM_Globalization.RMAApp_ExchangeRateSetup_Save_Button(driver),"Save Button On Exchange Rate Setup Page",logval);

						RMA_GenericUsages_Utility.RMA_StaticWait(10, logval,"Wait Is Added Window is Getting Opened");
						driver.switchTo().window(Handle);
						RMA_GenericUsages_Utility.RMA_DynamicFrameSwitching("MultiCurrencySetup");
						if(!Source.equals(Destination)){
							int test1 = i+1;

							destination_Source_Excahnge_Rate = driver.findElement(By.id("MultiCurrencySetupGrid_gvData_ctl11_"+test1+"")).getText();
							Rate1 = Double.parseDouble(destination_Source_Excahnge_Rate);
							found_value=1;
						}
						else{
							Rate1 = Double.parseDouble(destination_Source_Excahnge_Rate);
			...
			if(found_value==1){
				...
				if (logval == 0)
				{
					parentlogger.log(LogStatus.PASS, "Expected Values : " +"Source Currency "+color.RMA_ChangeColor_Utility(Source,2)+"Destination Currrency "+color.RMA_ChangeColor_Utility(Destination,2)+"Exchange Rate Exists" +color.RMA_ChangeColor_Utility(SearchedActualRate,2) +" , On Exchange Rate Setup Screen "+", And Hence Verification Is Successfull");
					parentlogger.log(LogStatus.PASS, "Expected Values : " +"Destination Currency "+color.RMA_ChangeColor_Utility(Destination,2)+"Source Currrency "+color.RMA_ChangeColor_Utility(Source,2)+"Exchange Rate Exists" +color.RMA_ChangeColor_Utility(destination_Source_Excahnge_Rate,2) +" , On Exchange Rate Setup Screen "+", And Hence Verification Is Successfull");	
				}
				else
				{
					logger.log(LogStatus.PASS, "Expected Values : " +"Source Currency "+color.RMA_ChangeColor_Utility(Source,2)+"Destination Currrency "+color.RMA_ChangeColor_Utility(Destination,2)+"Exchange Rate Exists" +color.RMA_ChangeColor_Utility(SearchedActualRate,2) +" , On Exchange Rate Setup Screen "+", And Hence Verification Is Successfull");
			...
			if(found_value==0){ // For Adding New Code As The Same Is Not Present On Exchange Rate Setup Screen
				SearchedActualRate =SearchedActualRate.valueOf(Rate1);
				RMA_Navigation_Utility.RMA_ObjectActionsClick(RMA_Selenium_POM_Globalization.RMAApp_ExchangeRateSetup_Button_AddNew(driver),"AddNew Image Button On Exchange Rate Setup Page",logval);
				RMA_GenericUsages_Utility.RMA_StaticWait(20, logval,"WAit Is Added For Frame To Get Loaded");
				RMA_GenericUsages_Utility.RMA_WindowSwitchingBasedOnTitle("Add/Modify Currency");

				RMA_Input_Utility.RMA_SetTextValueandTabOut_Utility(RMA_Selenium_POM_Globalization.RMAApp_ExchangeRateSetup_Source_Currency_InputBox(driver) , "Currency Type field on Globalization tab", Source, logval);
				RMA_Input_Utility.RMA_SetTextValueandTabOut_Utility(RMA_Selenium_POM_Globalization.RMAApp_ExchangeRateSetup_Destination_Currency_InputBox(driver) , "Currency Type field on Globalization tab", Destination, logval);
				RMA_Input_Utility.RMA_SetTextValueandTabOut_Utility(RMA_Selenium_POM_Globalization.RMAApp_ExchangeRateSetup_ExchangeRate_InputBox(driver) , "Currency Type field on Globalization tab", SearchedActualRate, logval);
				RMA_Navigation_Utility.RMA_ObjectActionsClick(RMA_Selenium_POM_Globalization.RMAApp_ExchangeRateSetup_Save_Button(driver),"Save Button On Exchange Rate Setup Page",logval);

				RMA_GenericUsages_Utility.RMA_StaticWait(10, logval,"WAit");
				driver.switchTo().window(Handle);
				RMA_GenericUsages_Utility.RMA_DynamicFrameSwitching("MultiCurrencySetup");
				if(!Source.equals(Destination)){
					lastrowno =lastrowno+1;
					destination_Source_Excahnge_Rate = driver.findElement(By.id("MultiCurrencySetupGrid_gvData_ctl11_"+lastrowno+"")).getText();
					Rate1 = Double.parseDouble(destination_Source_Excahnge_Rate);
				}
				else{
					Rate1 =Double.parseDouble(SearchedActualRate);
					SearchedActualRate =SearchedActualRate.valueOf(Rate1);
				}
				found_value =2;
			}	
			if(found_value==2){
				if (logval == 0)
				{
					if(!Source.equals(Destination)){
						...
						parentlogger.log(LogStatus.PASS, "Expected Values : " +"Source Currency "+color.RMA_ChangeColor_Utility(Source,2)+"Destination Currrency "+color.RMA_ChangeColor_Utility(Destination,2)+"Exchange Rate Added" +color.RMA_ChangeColor_Utility(SearchedActualRate,2) +" , On Exchange Rate Setup Screen "+", And Hence Verification Is Successfull");
						parentlogger.log(LogStatus.PASS, "Expected Values : " +"Destination Currency "+color.RMA_ChangeColor_Utility(Destination,2)+"Source Currrency "+color.RMA_ChangeColor_Utility(Source,2)+"Exchange Rate Added" +color.RMA_ChangeColor_Utility(destination_Source_Excahnge_Rate,2) +" , On Exchange Rate Setup Screen "+", And Hence Verification Is Successfull");
					}
					else{
						parentlogger.log(LogStatus.PASS, "Expected Values : " +"Source Currency "+color.RMA_ChangeColor_Utility(Source,2)+"Destination Currrency "+color.RMA_ChangeColor_Utility(Destination,2)+"Exchange Rate Added" +color.RMA_ChangeColor_Utility(SearchedActualRate,2) +" , On Exchange Rate Setup Screen "+", And Hence Verification Is Successfull");
					}
	...
	//Revision			: 0.0 - ImteyazAhmad-12-29-2015                                 
	//============================================================================================
	private static Boolean RMA_CheckTextInRowExistsTablePartiallyAllCases(List<WebElement> TableCol , String SearchedText)
	{
		Boolean TextFound = true;
		Boolean TextNotFound = false;
		SearchedText = SearchedText.trim();
	...
	//============================================================================================
	//FunctionName          : RMA_VerifyRowDataForTwoColTablePartiallyAllCases
	//Description           : To Verify The Text In Particular Column of  A Table Based On The User Provided Two Search Value, 
	//Input Parameter       : Element: Table Element, SearchText1: Text1 To Be Searched,SearchText2: Text2 To Be Searched, ExpectedValue: Expected value To Be Verified,  ExpectedCol: Column In Which The Value Is To Be Verified, ObjDes: Name oF The Table
	//Revision              : 0.0 - ShrutiShruti-10-23-2019                                
	//============================================================================================
	public static void RMA_VerifyRowDataForTwoColTablePartiallyAllCases(WebElement Element,String SrchedText1, String SrchedText2, String ExpectedValue, int ExpectedCol, String ObjDes, int logval ) throws Exception,Error
	{
		try
		{
			Thread.sleep(4000);
			...
			loop:
				...
				{     
					...
					if (RMA_CheckTextInRowExistsTablePartiallyAllCases(TableCol,  SrchedText1) && RMA_CheckTextInRowExistsTablePartiallyAllCases(TableCol, SrchedText2))
					{ 
						String Text = TableCol.get(ExpectedCol-1).getText();
						//Text = Text.toUpperCase();
						if (Text.contains(ExpectedValue))
	...
	//=====================================================================================================================================================================================================================================================
	//FunctionName 		: RMA_VerifyCurrencySymbol_Utility
	//Description  		: Enables The User To Verify And Retrieve Currency Symbol Based on Currency Code Passed In Argument
	//Input Parameter 	: SearchedText: Provide Unique Currency Code Logval
	//Revision			: 0.0 - ParulSharma-11-04-2019                               
	//=====================================================================================================================================================================================================================================================
	public static String RMA_VerifyCurrencySymbol_Utility(String Currencytype,int logval ) throws Exception,Error		
	{
		...
		{	
			if (logval == 0)
			{
				...
				logger = reports.startTest("<span id='CurrencySymbol'>Retrieve Currency Symbol Based On Currency Type</span>","Retrieve Currency Symbol of Given Currency Type ");
			}
			...
			if(found_value==1){
				if (logval == 0)
				{
					System.out.println("Currency_symbol_Base in function"+Currency_symbol);
					parentlogger.log(LogStatus.PASS, "Expected Values : " +"Source Currency "+color.RMA_ChangeColor_Utility(Currencytype,2)+"Currency Symbol is"+color.RMA_ChangeColor_Utility(Currency_symbol,2) +", And Hence Verification Is Successfull");
				}
				else
				{
					System.out.println("Currency_symbol_Base 2 in function"+Currency_symbol);
					logger.log(LogStatus.PASS, "Expected Values : " +"Source Currency "+color.RMA_ChangeColor_Utility(Currencytype,2)+"Destination Currrency "+color.RMA_ChangeColor_Utility(Currency_symbol,2) +", And Hence Verification Is Successfull");
				}
			}
		}			
		catch(Exception|Error e)
	...
	//============================================================================================
	//FunctionName          : RMA_VerifyIsAlertPresent
	//Description           : To Verify Is Alert Present On Screen 
	//Input Parameter       : 
	//Revision              : 0.0 - DebasmitaPati-11-12-2019                                
	//============================================================================================

	public static boolean RMA_VerifyIsAlertPresent(){
		try{
	...
	//=====================================================================================================================================================================================================================================================
	//FunctionName 		: RMA_VerifyExchangeRate_Utility
	//Description  		: Enables The User To Verify If Exchange Rate Is Present On Exchange Rate setup Screen If Yes Then Delete All The Existing Rates
	//Input Parameter 	: No Parameters Are Passed In This Function Except Logval
	//Revision			: 0.0 - ParulSharma-11-23-2019                               
	//=====================================================================================================================================================================================================================================================
	public static void RMA_VerifyExchangeRate_Utility_Delete(int logval) throws Exception,Error		
	{
		int found_value=0; // To Know If Expected Exchange Rate Is Present On The Screen Or Not 
		String Handle = driver.getWindowHandle();//To Know Which Browser Window Is Opened when The Given Function Was Called
		try
		{					
			if (logval == 0)
			{
				parentlogger.log(LogStatus.INFO, "Enables The User To Delete All Exchange Rates Present On Exchange Rate Setup Screen");
			}else{	
				logger = reports.startTest("<span id='ExchangeRateSetup'>Delete All Exchange Rates Present On Exchange Rate Setup Screen</span>","Enables The User To Verify If Exchange Rate Is Present For Different Currency and Add The same If Record Is Not Present On Exchange Rate Setup Screen");
			}
			WebElement Table_body = RMA_Selenium_POM_Globalization.RMAApp_ExchangeRateSetup_ExistingRecord_Table(driver);
			List<WebElement>TotalRowsList =Table_body.findElements(By.tagName("tr"));
			System.out.println("row list is"+TotalRowsList.size());
			int k =TotalRowsList.size()-2;
			int j=0;
			for(int i=0;i<=k;i++)// Loop For Finding Whether Exchange Rate for Exists For Source And Destination Currency Or Not
			{
				String CurrentRowValue = driver.findElement(By.id("MultiCurrencySetupGrid_gvData_ctl07_"+j+"")).getText(); // For Extracting Value Of 1St Column Of The Given Row
				if(!CurrentRowValue.isEmpty()){//If Exchange Rate Exists Then Delete the same 
					RMA_Navigation_Utility.RMA_ObjectClick(RMA_Selenium_POM_Globalization.RMAApp_Globalization_Option_Button(driver,j), "option button clicked on exchange rate record",logval);
					RMA_Navigation_Utility.RMA_ObjectClick(RMA_Selenium_POM_Globalization.RMAApp_ExchangeRateSetup_Button_Delete(driver), "Delete button clicked on exchange rate record",logval);
			...
			if(found_value==1){
				if (logval == 0)
				{
					parentlogger.log(LogStatus.PASS, "Existing Exchange Rate Deleted Successfully On Exchange Rate Setup Screen ");
				}
				else
				{
					logger.log(LogStatus.PASS, "Existing Exchange Rate Deleted Successfully On Exchange Rate Setup Screen ");
				}
			}					
		}			
		catch(Exception|Error e)
	...
	//=====================================================================================================================================================================================================================================================
	//FunctionName 		: RMA_VerifyUXGridPartiallyValue_Utility
	//Description  		: Enables The User To Verify Two Expected Values In Two Different Columns,After Finding An Unique Row Based Upon Unique Searched Text Provided  
	//Input Parameter 	:  SearchedText: Provide Unique Text From UXGrid, UXGridDescription:GridDescription,ExpectedColHeadersToBeVerified: Expected Col Headers To Be Verified separated by ';'(e.g. Reserve Type, Balance on Financial Reserve Grid),ExpectedValuesToBeVerified:Expected Values To Be Verified (e.g. Reserve Type value, Balance value on Financial Reserve Grid),logval
	//Revision			: 0.0 - ShrutiShruti-07-15-2020                               
	//=====================================================================================================================================================================================================================================================
	public static void RMA_VerifyUXGridPartiallyValue_Utility(String SearchedText, String UXGridDescription,  String[] ExpectedColHeadersToBeVerified, Object[] ExpectedPartialValuesToBeVerified,int logval ) throws Exception,Error		
	{
		int FoundRow;// To find row id of search text
		String ActualText;
		int FoundCol;// to find column index of the given expected value
				...
				outerloop:
					for(int i = 0; i< UX_row.size(); i++) // run the loop to which DOM row matching with the given expected SearchedText
					{
						...
						for(int j=0; j< UX_column.size(); j++)// run the loop to which DOM column to the corresponding row matching with the given expected SearchedText
						{
							...
							SearchedTextActual =((JavascriptExecutor) driver).executeScript("return arguments[0].innerText;", UX_column.get(j) ).toString();
							System.out.println("SearchedTextActual"+SearchedTextActual);
							if (SearchedTextActual.trim().contains(SearchedText.trim()))// compare DOM column to the corresponding row with the given expected SearchedText
							{
								FoundRow = i;// Store row 
								break outerloop;
							}
