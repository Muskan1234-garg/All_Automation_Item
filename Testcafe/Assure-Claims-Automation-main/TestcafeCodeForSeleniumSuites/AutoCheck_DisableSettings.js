...
//Description      :
// 1. Payment Parameters -> Payment (Horizontal Menu) -> Void (Verticle Menu) -> Check 'Check Void Reason' -> Turn Off
// 2. Payment Parameters -> Payment (Horizontal Menu) -> Payee (Verticle Menu) -> Check 'Include Prefix on Payments' & Check 'Include Suffix on Payments' -> Turn Off
...
DataIUT.forEach(data => {
    fixture`AutoCheck_DisableSettings`.beforeEach(async t => {
        await t.wait(5000);
    ...
    test('AssueClaims_TurnOnSettingFromConfigManager_Test_01', async t => {
        ...
        await t.wait(5000);
        await Nav_Utility.AssureClaims_WebCheckBoxSelectConfigMgnr_Utility(PayParaSetUp_POM.AssureClaims_PaymentParameters_Chk_IncludePrefixOnPayments, "UnCheck", "Void code reason")
        await Nav_Utility.AssureClaims_WebCheckBoxSelectConfigMgnr_Utility(PayParaSetUp_POM.AssureClaims_PaymentParameters_Chk_IncludeSuffixOnPayments, "UnCheck", "Void code reason")
        await Nav_Utility.AssureClaims_ElementClick_Utility(ConfigMgr_POM.AssureClaims_ConfigManager_Btn_SaveBtn, "Save")
