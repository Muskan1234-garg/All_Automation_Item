...
DataIUT.forEach(data => {
    fixture`POST_OSHA_300`.beforeEach(async t => {  
        await t.wait(4000)
...
var Dt; 
var Tm; 
var LatestJob = data.OSHA_300_ReportName+'-'+data.Username+'-';
...
//Below script will select option By Org-Hierarchy in OSHA Information [By Organizational Hierarchy] and Use Year in OSHA Reporting Date Information [Explicit] Tabs
    ...
    await Nav_Utility.AssureClaims_VerifyAndClick_Utility(data.Report_Description,POM_OSHAcsv.AssureClaimsApp_OSHA_AvailableReports_Lst.withText(data.OSHA_300_ReportName), "OSHA 300 Report(where Job needs to Run)")//change report name & report description as per requirement
    await t.wait(2000)
    ...
    await Nav_Utility.AssureClaims_ElementClick_Utility(POM_OSHAcsv.AssureClaimsApp_OSHA_ReportingDateInfo_Option_RadioBtn('[value=useyear]').nth(0), "Option Use Year")
    ...
    await Nav_Utility.AssureClaims_ElementClick_Utility(POM_OSHAcsv.AssureClaimsApp_OSHA_ReportingDataOptions.nth(2)("eventbasedflag"),"Event Based Report")
    await Nav_Utility.AssureClaims_ElementClick_Utility(POM_OSHAcsv.AssureClaimsApp_OSHA_ReportingDataOptions.nth(3)("printoshadescflag"), "Use OSHA Description")
...
//Below script will select option By Org-Hierarchy in OSHA Information [By Organizational Hierarchy] and Use Date Range in OSHA Reporting Date Information [Explicit] Tabs
    ...
    await Nav_Utility.AssureClaims_ElementClick_Utility(POM_OSHAcsv.AssureClaimsApp_OSHA_OshaReportsQueue.withText("Available Reports"), "Available Reports")
    await Claims_Osha.AssureClaims_FrameSwitch_OSHAQueue("smreportsel")
    await Nav_Utility.AssureClaims_VerifyAndClick_Utility(data.Report_Description,POM_OSHAcsv.AssureClaimsApp_OSHA_AvailableReports_Lst.withText(data.OSHA_300_ReportName), "OSHA 300 Report(Job will Run under this Report)")//change report name & report description as per requirement
    await t.wait(2000)
