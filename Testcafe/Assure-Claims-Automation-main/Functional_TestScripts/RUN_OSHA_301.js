...
DataIUT.forEach(data => {
    fixture`POST_OSHA_301`.beforeEach(async t => {  
        await t.wait(4000)
...
var Dt; 
var Tm; 
var LatestJob = data.OSHA_301_ReportName+'-'+data.Username+'-';
...
//Below script will select option By Org-Hierarchy in OSHA Information [By Organizational Hierarchy] and Use Year in OSHA Reporting Date Information [Explicit] Tabs
    ...
    await Nav_Utility.AssureClaims_VerifyAndClick_Utility(data.Report_Description,POM_OSHAcsv.AssureClaimsApp_OSHA_AvailableReports_Lst.withText(data.OSHA_301_ReportName), "OSHA 301 Report(where Job needs to Run)")//change report name & report description as per requirement
    await t.wait(2000)
...
//Below script will select option By Org-Hierarchy in OSHA Information [By Organizational Hierarchy] and Use Date Range in OSHA Reporting Date Information [Explicit] Tabs
    ...
    await Nav_Utility.AssureClaims_ElementClick_Utility(POM_OSHAcsv.AssureClaimsApp_OSHA_OshaReportsQueue.withText("Available Reports"), "Available Reports")
    await Claims_Osha.AssureClaims_FrameSwitch_OSHAQueue("smreportsel")
    await Nav_Utility.AssureClaims_VerifyAndClick_Utility(data.Report_Description,POM_OSHAcsv.AssureClaimsApp_OSHA_AvailableReports_Lst.withText(data.OSHA_301_ReportName), "OSHA 301 Report(Job will Run under this Report)")//change report name & report description as per requirement
    await t.wait(2000)
...
//======================================Login Into Application with User started==================================================================
    ...
    await Claims_Osha.AssureClaims_OSHA_RunReport_Utility(data.Run_Report)//Change Run Report in DataFiles as per requirement
    await Claims_Osha.AssureClaims_OSHA_NotifyType_Utility(data.Notification_Type)//Change Notification Type in DataFiles as per requirement
    await Inpt_Utility.AssureClaims_SetValue_Utility(POM_OSHAcsv.AssureClaimsApp_OSHA_ReportInfo_EmailTo, data.Email_To, "Email/Emails")//Change Email To in DataFiles as per requirement and incase of multiple emails, sepatate them with semicolon ';'        await Inpt_Utility.AssureClaims_SetValue_Utility(POM_OSHAcsv.AssureClaimsApp_OSHA_ReportInfo_Optional_EmailMsg, data.Optional_Email_Message, "Optional Email Message")//Change Optional email message in DataFiles as per requirement
